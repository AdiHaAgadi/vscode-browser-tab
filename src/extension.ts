...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('antigravity-webview.open', () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('antigravity-webview.navigate', async () => {
      const input = await vscode.window.showInputBox({
        title: 'Navigate to URL',
        prompt: 'Enter a URL to open in the Antigravity Browser tab',
    ...
    vscode.window.registerTerminalLinkProvider({
      provideTerminalLinks(ctx, _token) {
        ...
        while ((m = URL_REGEX.exec(ctx.line)) !== null) {
          links.push(
            Object.assign(new (vscode.TerminalLink as any)(m.index, m[0].length, 'Open in Antigravity Browser'), {
              _url: m[0],
            })
          );
        }
